namespace Butterfly.system.objects.handler.description
{
    ...
    public interface IRestream<ValueType>
    {
        ...
        /// После продолжает выполнять все события в обьекте. В ответ придуд данные <typeparamref name="ValueType"/> в метод .awate .
        ...
        /// </summary>
        public IAsyncRestream<ValueType> async_output_to_echo<LocationEchoObjectType>(
            int pPollSize = 0, int pTimeDelay = 0, string pPollName = "")
            where LocationEchoObjectType : main.Object, new();

        /// <summary>
        /// Передает данные <typeparamref name="ValueType"/> в глобальный обьект <paramref name="pEchoName"/>, до первого узла.
        /// После продолжает выполнять все события в обьекте. В ответ придуд данные <typeparamref name="ValueType"/> в метод .awate .
        /// После чего цепочка событий продолжит свое выполнение.
        /// Используется в методе Construction(). 
        /// Связь устанавливается через проверку на соответвие типа.
        /// </summary>
        public IAsyncRestream<ValueType> async_output_to_echo(
        ...
        /// Связь устанавливается через проверку на соответвие типа.
        /// </summary>
        public IAsyncRestream<ReturnValueType> async_output_to_echo<LocationEchoObjectType, ReturnValueType>(
            int pPollSize = 0, int pTimeDelay = 0, string pPollName = "")
            where LocationEchoObjectType : main.Object, new();
